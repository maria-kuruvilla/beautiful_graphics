...
sum_species_data <- function(year, species,type,river,file_type){
  ...
  df_new_summarize <- df %>%
    group_by(WSPEName, StartDate, day_of_year, year, StartDate_OG) %>%
...
deviation <- function(coho_h,coho_w){
  for(i in 1:length(coho_h$sum_num_caught)){
    ...
    for(j in 1:length(coho_w$sum_num_caught)){
      ...
      if((coho_h$StartDate_OG[i] == coho_w$StartDate_OG[j]) && (coho_h$StartDate[i] == coho_w$StartDate[j])){
        #print(j)
...
p1 <- ggplot(data = chinook_combine,aes(x = sum_num_caught+1,y=wild_fish_deviation+10))+
  ...
  scale_y_log10(breaks = c(-10,0,10,20,30), labels = c(-20,-10,0,10,20))+
...
p2 <- ggplot(data = coho_combine,aes(x = sum_num_caught+1,y=wild_fish_deviation+10))+
  ...
  scale_y_log10(breaks = c(-10,0,10,20,30,40), labels = c(-20, -10,0,10,20,30), limits= c(2,60))+
